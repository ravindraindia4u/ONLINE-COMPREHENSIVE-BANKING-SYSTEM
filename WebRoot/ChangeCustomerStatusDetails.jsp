...
<table align="center" WIDTH="650" border="0" bordercolor="black" bgcolor="green">
    ...
    <%if(request.getParameter("status")!=null)out.print(request.getParameter("status"));%></strong> </span> 
       ...
       try{
       CoreHash ch=new CustomerDAO().getAllNewRequestedCustomersDetails(images,addressprofs,loginname);
 ...
 <table align="center" WIDTH="650" border="0" bordercolor="block" bgcolor="">         
        ...
        <%   while(em.hasMoreElements()){
             cm=(CustomerModel)em.nextElement(); %> 
             <tr>
   
    <td value="<%=cm.getCustid()%>"><a href="SeeCustPersonalProfile.jsp?custid=<%=cm.getCustid()%>"><%=cm.getCustfname()%> </a></td>
      <td><%=cm.getDoj()%></td>
    <td><%=cm.getAddress()%></td>   
    <td value="<%=cm.getCustid()%>.gif"><img src="addressprofs/<%=cm.getCustid()%>.gif" height=20 width=20></td>
   <td><%=cm.getEmailid()%></td> 
    <td><%=cm.getPhno()%></td> 
   <td value="<%=cm.getCustid()%>.gif"><img src="userimages/<%=cm.getCustid()%>.gif" height=20 width=20></td>
     <td value=""><a href="Accept.jsp?custid=<%=cm.getCustid()%>">Accept </a></td>|
      <td value=""><a href="Reject.jsp?custid=<%=cm.getCustid()%>">Block </a></td>
