...
<table align="center" WIDTH="650" border="0" bordercolor="black" bgcolor="green">
      <th colspan="15" bgcolor=""><center><b><font color="white">CUSTOMERS LOAN  REQUEST DETAILS</font></center></th>
      <tr rowspan="4"></tr>
    ...
    <%if(request.getParameter("status")!=null)out.print(request.getParameter("status"));%></strong> </span> 
      ...
      try{
     CoreHash ch=new CardDAO().getAllCustCardRequests();
      ...
      <tr>
       <td>Card&nbsp;Number</td>
        ...
        <%   while(em.hasMoreElements()){
             am=(CardModel)em.nextElement(); %>    
        <tr> 
            <td><%=am.getCardno()%></td> 
            <td value="<%=am.getCustid()%>"><%=am.getCustname()%> </td>  
            <td><%=am.getCustaccno()%></td> 
            <td value="<%=am.getCardtypeid()%>"><%=am.getCardtypename()%></td>
            <td><%=am.getPinno()%></td> 
            <td><%=am.getApplydate()%></td>  
            <td value=""><a href="CustCardAccept.jsp?custid=<%=am.getCustid()%>">Accept </a></td>
            <td value=""><a href="CustCardReject.jsp?custid=<%=am.getCustid()%>">Reject </a></td>
